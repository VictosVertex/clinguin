...
export class AttributeHelperService {
  ...
  setDrag(html: HTMLElement, attributes: AttributeDto[]) {
    let draggable_as = this.findGetAttributeValue(
      "draggable_as",
      attributes,
      "00"
    );
  ...
  setBorderHelper(html: HTMLElement, attributes: AttributeDto[]) {
    let borderWidth = Number(
      this.findGetAttributeValue("border_width", attributes, "0")
    );
    let borderColor = this.findGetAttributeValue(
      "border_color",
      attributes,
      "red"
    );
  ...
  setBorder(
    ...
    if (borderWidth > 0) {
      html.style.border =
        String(borderWidth) + "px " + borderStyle + " " + borderColor;
    }
  }
  ...
  setGrid(html: HTMLElement, attributes: AttributeDto[]) {
    ...
    if (gridRowStart != null) {
      ...
      html.style.gridRow =
        String(gridRowStartN) + "/" + "span " + String(gridRowSpanN);
    }

    if (gridColumnStart != null) {
      let gridColumnStartN = Number(gridColumnStart.value) + 1;

      html.style.gridColumn =
        String(gridColumnStartN) + "/" + "span " + String(gridColumnSpanN);
    }
  }

  setHover(html: HTMLElement, attributes: AttributeDto[]) {
    let onHover = this.findGetAttributeValue("on_hover", attributes, "false");
    let onHoverBackgroundColor = this.findGetAttributeValue(
      "on_hover_background_color",
      attributes,
      "white"
    );
    let onHoverForegroundColor = this.findGetAttributeValue(
      "on_hover_foreground_color",
      attributes,
      "black"
    );
    let onHoverBorderColor = this.findGetAttributeValue(
      "on_hover_border_color",
      attributes,
      "white"
    );
    let backgroundColor = this.findGetAttributeValue(
      "background_color",
      attributes,
      "white"
    );
    let foregroundColor = this.findGetAttributeValue(
      "foreground_color",
      attributes,
      "black"
    );
  ...
  setAbsoulteRelativePositions(
    parentChildLayout: string,
    html: HTMLElement,
    child: ElementDto
  ) {
    let posX = Number(
      this.findGetAttributeValue("pos_x", child.attributes, "-1")
    );
    let posY = Number(
      this.findGetAttributeValue("pos_y", child.attributes, "-1")
    );
    ...
    if (
      (posY >= 0 || posX >= 0) &&
      (parentChildLayout == "absstatic" || parentChildLayout == "relstatic")
    ) {
  ...
  setVisibility(html: HTMLElement, attributes: AttributeDto[]) {
    ...
    if (visibilityAttribute != null) {
      if (
        visibilityAttribute.value == "hidden" ||
        visibilityAttribute.value == "collapse"
      ) {
        html.style.visibility = "collapse";
      } else if (
        visibilityAttribute.value == "shown" ||
        visibilityAttribute.value == "visible"
      ) {
