...
function hideAllContextMenus(): boolean {
  ...
  contextMenuService.contextMenus.forEach(
    (item: { key: string; contextMenu: ElementDto }) => {
...
function handleUpdate(when: WhenDto, event: Event | null) {
  ...
  let elementLookup: ElementLookupDto | null =
    elementLookupService.getElement(id);

  if (elementLookup != null) {
    if (elementLookup.element.type == "context_menu" && event != null) {
      if (key != "visibility" || (value != "visible" && value != "shown")) {
        console.error(
          "For updates to context menu only tuples of form (_,visibility,visible) are valid, but got: " +
            id +
            "," +
            key +
            "," +
    ...
    if (elementLookup.object != null) {
      if ("setAttributes" in elementLookup.object) {
        if (
          elementLookup.object.setAttributes != undefined &&
          typeof elementLookup.object.setAttributes === "function"
        ) {
    ...
    if (
      elementLookup.object == null &&
      elementLookup.element.type.startsWith("svg") &&
      elementLookup.tagHtml != null
    ) {
    ...
    if (elementLookup.tagHtml != null) {
      ...
      childBearerService.setChildTagAttributes(
        elementLookup.tagHtml,
        elementLookup.element
      );
    }
  } else {
    console.log(
      "COULD NOT FIND ELEMENT FOR when:" + id + "::" + key + "::" + value
    );
    console.log(when);
  }
}
...
function replaceContext(operation_string: string) {
  ...
  let regex =
    /_context_value\((?:"([^"]*)"|(\w+)|(\w+\(\s*(?:"[^"]*"|\w+)\s*\)))(?:,\s*(?:"([^"]*)"|(\w+)|(\w+\(\s*("[^"]*"|\w+)\s*\))))?(?:,\s*(?:"([^"]*)"|(\w+)|(\w+\(\s*(?:"[^"]*"|\w+)\s*\))))?\)/g;
  // ^(\w+)$|^(\w+\(\s*(?:"[^"]*"|\w+)\s*\))
  let regex_const =
    /^(\w+)$|^(\w+\((?:"([^"]*)"|(\w+)|(\w+\(\s*(?:"[^"]*"|\w+)\s*\)))((?:,\s*(?:"([^"]*)"|(\w+)|(\w+\(\s*("[^"]*"|\w+)\s*\))))?)*\))$/;
  ...
  while (match != null) {
    ...
    let isQuoted =
      new_value.length > 1 && new_value[0] == '"' && new_value.slice(-1) == '"';
    ...
    if (match_type != null) {
      if (match_type != "str" && match_type != "int" && match_type != "const") {
        throw new Error(
          "Not a valid type " + match_type + ". Should be str, int or const."
        );
      }
      ...
      if (match_type == "const" && !isConst) {
        throw new Error(
          "Expected a constant that can be parsed to an atom, but got: " +
            new_value
        );
      }
    }
...
function handleContext(when: WhenDto, event: Event | null) {
  ...
  if (operation[0] == "(") {
    ...
    if (splits.length >= 2) {
      ...
      if (
        ...
        event.target.getAttribute("name") == "range-min"
      ) {
        contextService.addContext(`${key}_min`, value);
      } else if (
        event?.target instanceof HTMLElement &&
        event.target.getAttribute("name") == "range-max"
      ) {
        contextService.addContext(`${key}_max`, value);
      } else {
  ...
  let message =
    "The value of context event should be a tuple of size 2, but got " +
    when.operation;
...
export class CallBackHelperService {
  ...
  findCallback(action: string, callbacks: WhenDto[]): WhenDto | null {
    let value = null;
    let index = callbacks.findIndex(
      (callback) => callback.actionType == action
    );
    if (index >= 0) {
    ...
    if (allEvents.length > 0 && htmlEventName != "") {
      if (supportedAttributeName == "load") {
        allEvents.forEach((when: WhenDto) => {
          ...
          if (
            when.interactionType == "call" ||
            when.interactionType == "callback"
          ) {
            // console.log("call")
            try {
      ...
      html.addEventListener(htmlEventName, function (event: Event) {
        ...
        const call = allEvents.filter(
          (w) => w.interactionType == "call" || w.interactionType == "callback"
        );
